...
# ------------------------------------------------------------------------------------
# STEP 1: RUN MOOSEZ SEGMENTATION
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------
# STEP 2: LOAD TUMOUR SEGMENTATION, COMPUTE CENTROID  AND COMPUTE THE VOLUME OF THE TUMOUR& SAVE AS FCSV
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------
# STEP 4: CANDIDATE DIRECTION SAMPLE + ENTRY POINT
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------
#   SAVE FCSV FOR SLICER OF THE CANDIDATE PATHS ( ALL PATHS INCLUDING COLLISION PATHS
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------
# STEP 6: COLLISION CHECKING (ORGANS + RIBS)
# ------------------------------------------------------------------------------------
...
def path_has_collision(entry_mm, target_mm,
    ...
    ANY segmented organ (label > 0) or ANY ribs voxel (label > 0).
    """
    entry = np.array(entry_mm, dtype=float)
    target = np.array(target_mm, dtype=float)
    ...
    for t in np.linspace(0.0, 1.0, num_samples):
        p = entry + t * (target - entry)  # point in mm
        ...
        if not (_is_inside_index(idx_org, size) and _is_inside_index(idx_rib, size)):
            # outside volume -> treat as collision (conservative)
            return True

        org_label  = int(organs_arr[idx_org[2], idx_org[1], idx_org[0]])
        ribs_label = int(ribs_arr[idx_rib[2], idx_rib[1], idx_rib[0]])

        if org_label > 0 or ribs_label > 0:
            return True
...
#-----------------------------------------------------------------------------------------------------
#RANKED BEST 5 COLLISION FREE PATHS
#---------------------------------------------------------------------------------------------------------
    # Input and output files
    input_fcsv = r"" # give your input fcsv file dir 
    output_fcsv = r"" # give your output file dir 
